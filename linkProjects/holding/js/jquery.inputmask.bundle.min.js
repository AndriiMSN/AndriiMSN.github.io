    ...
    function generateMaskSets(opts) {
      ...
      function getMaskTemplate(mask) {
        ...
        var singleMask = $.map(mask.split(""), function(element, index) {
          ...
          if (element == opts.escapeChar) {
            ...
            (element != opts.optionalmarker.start &&
              element != opts.optionalmarker.end) ||
            escaped
          ) {
            ...
            if (maskdef && !escaped) {
              for (var i = 0; i < maskdef.cardinality; i++) {
                outElem.push(
                  opts.placeholder.charAt(
                    (outCount + i) % opts.placeholder.length
                  )
                );
              }
            } else {
      ...
      function splitFirstOptionalEndPart(maskPart) {
        ...
        for (var i = 0; i < mpl; i++) {
          ...
          if (
            optionalStartMarkers > 0 &&
            optionalStartMarkers == optionalEndMarkers
          )
            break;
        }
      ...
      function generateMask(maskPrefix, maskPart, metadata) {
        ...
        if (masks.length > 1) {
          ...
          newMask =
            maskPrefix + masks[0] + (maskParts.length > 1 ? maskParts[1] : "");
          if ($.inArray(newMask, genmasks) == -1 && newMask != "") {
            genmasks.push(newMask);
            maskTemplate = getMaskTemplate(newMask);
            ms.push({
          ...
          if (
            maskParts.length > 1 &&
            splitFirstOptionalStartPart(maskParts[1]).length > 1
          ) {
            generateMask(
              maskPrefix + masks[0] + markOptional(masks[1]),
              maskParts[1],
              metadata
            );
    ...
    function maskScope(masksets, activeMasksetIndex, opts, actionObj) {
      ...
      function isValid(pos, c, strict) {
        ...
        function _isValid(position, activeMaskset, c, strict) {
          ...
          for (
            var i = activeMaskset["tests"][testPos].cardinality;
            i > loopend;
            i--
          ) {
          ...
          return activeMaskset["tests"][testPos].fn != null
            ? activeMaskset["tests"][testPos].fn.test(
                chrs,
                buffer,
                position,
                strict,
          ...
          if (hasValidActual) {
            ...
            $.each(results, function(ndx, rslt) {
              if (
                $.inArray(rslt["activeMasksetIndex"], maskForwards) != -1 &&
                (rslt["result"] !== false) &
                  (lowestPos == -1 || lowestPos > rslt["result"]["pos"])
              ) {
        ...
        $.each(masksets, function(index, value) {
          if (typeof value == "object") {
            ...
            if (lvp == actualLVP) {
              ...
              if (
                !isMask(maskPos) &&
                !_isValid(maskPos, getActiveMaskSet(), c, strict)
              ) {
                var maxForward = seekNext(maskPos) - maskPos;
                for (var fw = 0; fw < maxForward; fw++) {
                  if (
                    _isValid(++maskPos, getActiveMaskSet(), c, strict) !== false
                  )
                    break;
                }
      ...
      function determineActiveMasksetIndex() {
        ...
        $.each(masksets, function(index, value) {
          if (typeof value == "object") {
            ...
            if (
              ...
              highestValid["lastValidPosition"]
            ) {
              highestValid["activeMasksetIndex"] = index;
              highestValid["lastValidPosition"] = getActiveMaskSet()[
                "lastValidPosition"
              ];
              highestValid["next"] = seekNext(
                getActiveMaskSet()["lastValidPosition"]
              );
            } else if (
              getActiveMaskSet()["lastValidPosition"] ==
        ...
        activeMasksetIndex =
          ...
          masksets[currentMasksetIndex]["lastValidPosition"] ==
            highestValid["lastValidPosition"]
            ? currentMasksetIndex
      ...
      function clearBuffer(buffer, start, end, stripNomasks) {
        for (
          var i = start, maskL = getMaskLength();
          i < end && i < maskL;
          i++
        ) {
      ...
      function setReTargetPlaceHolder(buffer, pos) {
        ...
        setBufferElement(
          buffer,
          pos,
          getBufferElement(getActiveBufferTemplate(), testPos)
        );
      }
      ...
      function checkVal(input, writeOut, strict, nptvl, intelliCheck) {
        var inputValue =
          nptvl != undefined
            ? nptvl.slice()
            : truncateInput(input._valueGet()).split("");
        ...
        $.each(inputValue, function(ndx, charCode) {
          if (intelliCheck === true) {
            ...
            if (
              $.inArray(
                charCode,
                getActiveBufferTemplate().slice(lvp + 1, pos)
              ) == -1
            ) {
              keypressEvent.call(
                input,
        ...
        if (strict === true && getActiveMaskSet()["p"] != -1) {
          getActiveMaskSet()["lastValidPosition"] = seekPrevious(
            getActiveMaskSet()["p"]
          );
        }
      }
      ...
      function truncateInput(inputValue) {
        return inputValue.replace(
          new RegExp(
            "(" + escapeRegex(getActiveBufferTemplate().join("")) + ")*$"
          ),
          ""
        );
      }
      ...
      function unmaskedvalue($input, skipDatepickerCheck) {
        if (
          ...
          var umValue = $.map(getActiveBuffer(), function(element, index) {
            return isMask(index) && isValid(index, element, true)
              ? element
              : null;
          });
      ...
      function patchValueProperty(npt) {
        ...
        if (valueProperty && valueProperty.get) {
          ...
          if (
            $.valHooks.text == undefined ||
            $.valHooks.text.inputmaskpatch != true
          ) {
            var valueGet =
      ...
      function shiftR(start, end, c) {
        ...
        if (
          c != undefined &&
          getBufferElement(buffer, start) == getPlaceHolder(start)
        )
      ...
      function HandleRemove(input, k, pos) {
        ...
        if (pos.begin == pos.end) {
          var posBegin =
            k == opts.keyCode.BACKSPACE ? pos.begin - 1 : pos.begin;
          if (
            opts.isNumeric &&
            opts.radixPoint != "" &&
            ...
            pos.begin =
              getActiveBuffer().length - 1 == posBegin
                ? pos.begin
                : k == opts.keyCode.BACKSPACE
                ? posBegin
                : seekNext(posBegin);
      ...
      function keydownEvent(e) {
        ...
        if (
          ...
          (k == opts.keyCode.HOME && !e.shiftKey) ||
          k == opts.keyCode.PAGE_UP
        ) {
      ...
      function keypressEvent(e, checkval, k, writeOut, strict, ndx) {
        ...
        if (
          ...
          if (k) {
            ...
            var radixPosition = getActiveBuffer()
              .join("")
            ...
            if (writeOut !== false) {
              ...
              if (result != undefined) {
                ...
                if (
                  getActiveMaskSet()["writeOutBuffer"] &&
                  result["result"] !== false
                ) {
                  var buffer = getActiveBuffer();
                  ...
                  getActiveMaskSet()["buffer"] = getActiveMaskSet()[
                    "undoBuffer"
                  ].split("");
                }
              }
          ...
          if (
            ...
            if (
              ...
              caret(
                input,
                TranslatePosition(0),
                TranslatePosition(getMaskLength())
              );
          }
        }
      }
      ...
      function mask(el) {
        ...
        if ($el.is(":input")) {
          ...
          $el
            ...
            .bind("mouseleave.inputmask", function() {
              ...
              if (opts.clearMaskOnLostFocus) {
                if (
                  ...
                  if (
                    input._valueGet() == getActiveBufferTemplate().join("") ||
                    input._valueGet() == ""
                  )
                    input._valueSet("");
                  else {
            ...
            .bind(
              pasteEvent + ".inputmask dragdrop.inputmask drop.inputmask",
              function(e) {
                if (skipInputEvent === true) {
                  skipInputEvent = false;
                  return true;
                ...
                if (
                  ...
                  input._valueGet().length <= getMaskLength()
                ) {
                  return true;
                }
                setTimeout(function() {
          ...
          if (activeElement === el) {
            ...
            if (
              getActiveBuffer().join("") == getActiveBufferTemplate().join("")
            ) {
              el._valueSet("");
            } else {
        ...
        switch (fn) {
          ...
          case "unmaskedvalue":
            ...
            if ($input.data("_inputmask")) {
              ...
              activeMasksetIndex = $input.data("_inputmask")[
                "activeMasksetIndex"
              ];
          ...
          case "getemptymask":
            if (this.data("_inputmask")) {
              ...
              activeMasksetIndex = this.data("_inputmask")[
                "activeMasksetIndex"
              ];
